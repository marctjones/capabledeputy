...
`capdep audit storage-shape` exits 0 on a clean v6 store and
non-zero on one carrying flat-legacy rows (sessions with empty
axis_a/axis_b after migration).
"""
...
def _make_clean_v6_store(tmp_path: Path) -> Path:
    """Create a minimal v6 sessions store with one well-formed row.
    ...
    conn.execute(
        "INSERT INTO sessions (id, axis_a, axis_b, axis_d) VALUES (?, ?, ?, ?)",
        ("s1", '[{"category":"personal"}]', '[{"level":"principal-direct"}]', "{}"),
    )
    conn.commit()
    conn.close()
    return db
...
def _make_legacy_v6_store(tmp_path: Path) -> Path:
    """A v6 store with a session whose axis_a/axis_b are still empty
    ...
    conn.execute(
        "INSERT INTO sessions (id, label_set, axis_a, axis_b) VALUES (?, ?, ?, ?)",
        ("legacy", '["confidential.personal"]', "[]", "[]"),
    )
    conn.commit()
    conn.close()
    return db
...
def test_audit_passes_on_clean_v6_store(tmp_path: Path) -> None:
