...
- [ ] T019 [US2] Invalidate any pending approval where `inert(approval.capability_requested)` (reuse the existing `ApprovalRequest.capability_requested` field — no new linkage), and emit `capability.cascade_revoked` (originating audit_id, trigger, affected descendant audit_ids + sessions) in src/capabledeputy/approval/queue.py
