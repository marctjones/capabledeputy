...
- **Unlabeled reversibility**: an action with no applicable reversibility label MUST be treated as irreversible and gated accordingly, never optimistically executed as if reversible.
...
- **FR-037**: Reversibility MUST be a first-class, human-declared, AI-read-only **label** attachable to (a) effect classes (a default, e.g. `COMMUNICATE`/egress = irreversible), (b) resources/systems/targets (e.g. a git-backed repo = reversible; an external mailbox = irreversible), and (c) channels. An action's *effective reversibility* MUST be the **most-restrictive (least reversible)** across the applicable effect default, target-resource label, and channel label. An action whose reversibility is unlabeled or indeterminate MUST be treated as **irreversible** (fail-closed, Principle VI), never assumed reversible. Effective reversibility is the single shared input consumed by reversibility-weighted gating (FR-019), the risk-envelope key (FR-030), the optimistic-execution boundary (FR-034), and approval criticality/grouping (FR-035). Reversibility labels are AI-read-only (FR-012); only a human may declare or change one, and each carries assignment provenance (FR-022).
...
- **Reversibility Label**: human-declared, AI-read-only reversibility class attachable to an effect class, a resource/system/target, or a channel; composed most-restrictive-wins; unlabeled ⇒ irreversible; carries assignment provenance (FR-037).
...
- **SC-015**: Reversibility resolves correctly 100% in the fixtures: a write to a git-backed resource resolves `reversible`; an email/egress to an external counterparty resolves `irreversible`; an unlabeled action resolves `irreversible`; effective reversibility always equals the most-restrictive of the contributing labels; 0 actions are treated as reversible without a human-declared label supporting it.
