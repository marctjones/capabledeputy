...
**Status:** Alpha — **v0.22.0 released**. v0.22.0 makes CapDep practical as a supervised macOS and Google Workspace personal assistant: official Google Workspace remote MCP servers use CapDep's native OAuth2/PKCE token flow, bounded Apple Mail/Keynote/Pages/Numbers/macOS automation servers are first-class, browser and app automation capability kinds are granular, and the personal-assistant preset defaults to macOS paths plus Google/Apple read-and-draft workflows. v0.21.0 aligned CapDep's flow-pattern enforcement with its security model. v0.20.0 made local Apple Silicon inference a first-class default with MLX and `Qwen/Qwen3-4B-MLX-4bit`. See [CHANGELOG.md](CHANGELOG.md) for released changes and [ROADMAP.md](ROADMAP.md) for longer-term planning.
