...
**Status:** Alpha — **v0.20.0 released**. v0.20.0 makes local Apple Silicon inference a first-class default: CapDep now selects the MLX backend on macOS arm64 and uses **`Qwen/Qwen3-4B-MLX-4bit`** for the planning model unless the operator explicitly chooses another backend. It also adds optional Claude CLI and LiteLLM backend selection, strips model thinking blocks before parsing/rendering, raises the Python baseline to 3.14, hardens macOS platform behavior, and refactors the policy/tool boundary around canonical policy context, policy hooks, reference handles, and explicit source-flow enforcement. v0.19.0 added the greenfield **inline console** (`capdep ui`) with quarantine-rendered untrusted content, typed engine decision cards, armed approval interaction, a deny-oriented kill switch, and `/flow` lineage visibility. See [CHANGELOG.md](CHANGELOG.md) for what shipped and [ROADMAP.md](ROADMAP.md) for the plan.
