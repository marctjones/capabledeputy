...
**Status:** Alpha — **v0.44.0 released** — SKILL.md interoperability, sandbox-only skill script execution, CommonMark rendering parity, and daemon-owned local media/model operations. Recent releases also added CapDepMac prompt queueing/recovery, safe scripting workflows, generated-image session artifacts, daemon-owned setup, connector admission, rich chat media, local model routing, background automation coordination, memory/retention/compaction, and MCP workflow templates. See [CHANGELOG.md](CHANGELOG.md) for released changes and [ROADMAP.md](ROADMAP.md) for longer-term planning.
